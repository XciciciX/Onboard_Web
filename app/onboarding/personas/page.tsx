...
export default function PersonasPage() {
  ...
  const [editingFilter, setEditingFilter] = useState<PersonaFilter | null>(null);
  ...
  const [newPersonaData, setNewPersonaData] = useState<{id: string, title: string, key: string} | null>(null);
  ...
  // Get the currently selected persona object
  const currentPersona = localPersona || personas.find(p => p.id === selectedPersona);
  ...
  useEffect(() => {
    async function fetchPersonas() {
      ...
      try {
        ...
        showError('Error loading personas. Please try again.');
        console.error(err);
      } finally {
        setIsLoading(false);
      }
  ...
  const handlePersonaClick = (personaId: string) => {
    ...
    if (selectedPersona === personaId) {
      setSelectedPersona(null); // Close if already selected
      ...
      if (persona) {
        ...
        setKey(persona.key);
        // Create a local copy of the persona for editing
        setLocalPersona(JSON.parse(JSON.stringify(persona)));
      }
      ...
      if (persona && persona.filterGroups.length > 0) {
        setSelectedGroupId(persona.filterGroups[0].id);
      } else {
  ...
  // Update the handleCreatePersona function
  const handleCreatePersona = async () => {
    ...
    // Generate a temporary ID for the new persona
    const tempId = Date.now().toString();
    
    // Generate random contacts
    const randomContacts = generateRandomContacts();
    
    // Create a new local persona
    const newPersona = {
      id: tempId,
      title: 'New Persona',
      key: 'new_persona',
    ...
    // Set up the new persona in the UI without sending to the API yet
    setSelectedPersona(tempId);
    setTitle(newPersona.title);
    setKey(newPersona.key);
    setFilterType('Title');
    setFilterOperator('Contains');
    setFilterValue('');
    setEditingFilter(null);
  ...
  const handleAddFilterGroup = () => {
    ...
    setLocalPersona({
      ...localPersona,
      filterGroups: [...localPersona.filterGroups, newGroup]
    });
  ...
  const handleChangeGroupOperator = (groupId: string, operator: 'AND' | 'OR') => {
    if (!localPersona) return;
    
    setLocalPersona({
      ...localPersona,
      filterGroups: localPersona.filterGroups.map(group => 
  ...
  // Delete a filter group locally
  const handleDeleteFilterGroup = (groupId: string) => {
    if (!localPersona) return;
    
    setLocalPersona({
      ...localPersona,
      filterGroups: localPersona.filterGroups.filter(group => group.id !== groupId)
    });
    
    if (selectedGroupId === groupId) {
      // Select another group if available
      if (localPersona.filterGroups.length > 1) {
        const remainingGroups = localPersona.filterGroups.filter(g => g.id !== groupId);
  ...
  // Add a filter locally
  const handleAddFilter = () => {
    if (!localPersona || !selectedGroupId || !filterValue.trim()) return;
  ...
  const handleUpdateFilter = () => {
    if (!localPersona || !selectedGroupId || !editingFilter || !filterValue.trim()) return;
  ...
  const handleDeleteFilter = (filterId: string) => {
    if (!localPersona || !selectedGroupId) return;
  ...
  // Edit a filter
  const handleEditFilter = (filter: PersonaFilter) => {
  ...
  const handleSubmitPersona = async () => {
    if (!localPersona || !title.trim() || !key.trim()) {
    ...
    try {
      ...
      if (isNewPersona) {
        ...
        console.log(`Updating persona ${selectedPersona} with full data:`, JSON.stringify(updatedPersona));
        response = await fetch(`/api/personas/${selectedPersona}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to ${isNewPersona ? 'create' : 'update'} persona: ${response.status} ${response.statusText}`);
      }
      ...
      if (isNewPersona) {
        ...
        setPersonas(prevPersonas => {
          ...
          const filteredPersonas = prevPersonas.filter(p => p.id !== selectedPersona);
      ...
      console.error('Error in handleSubmitPersona:', err);
      showError(`Error ${isNewPersona ? 'creating' : 'updating'} persona. Please try again.`);
    }
  };
  ...
  const handleDeletePersona = async () => {
    ...
    try {
      ...
      if (isNewPersona) {
        setPersonas(personas.filter(p => p.id !== selectedPersona));
        setSelectedPersona(null);
        setLocalPersona(null);
        setIsNewPersona(false);
        setNewPersonaData(null);
      ...
      const response = await fetch(`/api/personas/${selectedPersona}`, {
        method: 'DELETE',
      });
  ...
  return (
    <form className="p-6 max-w-6xl mx-auto">
      ...
      <p className="mb-6 text-gray-400">You will be able to filter Pursuit data by Personas</p>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        {/* Personas list and details in a two-column layout */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Personas list */}
          <div className="space-y-1">
            <h3 className="text-sm font-medium text-gray-400">Personas ({personas.length})</h3>
              ...
              {personas.map(persona => (
                <div 
                  ...
                  className={`flex justify-between items-center p-2 rounded cursor-pointer ${
                    selectedPersona === persona.id ? 'bg-blue-900/50 border border-blue-500' : 'hover:bg-gray-800'
                  }`}
                >
                  <div>
                    <div className="font-medium">{persona.title}</div>
                    <div className="text-xs text-gray-400">{persona.contacts} contacts</div>
                  </div>
          ...
          {/* Item settings - only shown when a persona is selected */}
          {currentPersona && (
            <div className="md:col-span-2 space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium">Item settings</h3>
                <div className="text-sm text-gray-400">
                  {currentPersona.contacts} Contacts meet criteria
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="text-xs text-gray-400 mt-1">
                    Value that will appear in your connected CRM
                  </div>
                </div>
                  ...
                  <div className="flex justify-between mb-3">
                    <div className="flex gap-2">
                      <select
                        ...
                        disabled={currentPersona.filterGroups.length === 0}
                      >
                        {currentPersona.filterGroups.length === 0 ? (
                          <option value="">No groups</option>
                        ) : (
                          <>
                            <option value="">Select group</option>
                            {currentPersona.filterGroups.map((group, index) => (
                ...
                <div className="mt-4 space-y-4">
                  {currentPersona.filterGroups.map((group, groupIndex) => (
                      ...
                      <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            onChange={(e) => handleChangeGroupOperator(group.id, e.target.value as 'AND' | 'OR')}
              ...
              <div className="flex justify-between pt-4">
                ...
                >
                  {isNewPersona ? 'Submit' : 'Apply changes'}
                </button>
              </div>
            </div>
          )}
